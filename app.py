...
class Project(db.Model):
    ...
    messages = db.relationship("ProjectMessage", backref="project", cascade="all, delete-orphan", order_by="ProjectMessage.id")
...
class ProjectMessage(db.Model):
    ...
    author_role = db.Column(db.String(30), nullable=False)
...
def can_access_project(project):
    u = current_user()
    return bool(u and (u.role == "huys_admin" or u.company_id == project.company_id))
...
def create_partner(company_name, name, email, password):
    if Company.query.filter_by(name=company_name).first():
        return "Dit partnerbedrijf bestaat al."
    if User.query.filter_by(email=email).first():
        return "Dit e-mailadres is al in gebruik."
    company = Company(name=company_name)
    db.session.add(company)
    db.session.flush()
    db.session.add(User(company_id=company.id, name=name, email=email, password_hash=generate_password_hash(password), role="partner_admin"))
    db.session.commit()
    return None
